...
public class Player {
    ...
    public ArrayList<Double> outToDoubleArray(MLData out){
        ArrayList<Double> binaryOutput=new ArrayList<Double>();
        for(int i=0;i<out.size();i++){
            if(out.getData(i)>=0.5){
                binaryOutput.add(1.0);
            }
            else{
                binaryOutput.add(0.0);
            }
        }
        return binaryOutput;
    }

    public void addInput(MLData out){
        ArrayList<Double> outArray=outToDoubleArray(out);
        input.add(outArray);
    }
    ...
    public void train() {
        int hiddenUnits = network.getLayerNeuronCount(0) + 1; //construct new net with count one more than previous
        ...
        */
            ...
            do {
                ...
                epoch++;
           } while (train.getError() > .001 && epoch<100000);
...
*/
    public boolean hasClassificationError(BasicNetwork netw, MLDataSet trainSet){
        ...
        for(MLDataPair pair:trainSet){
            ...
            System.out.println("index to check: "+index+", actual index: "+currIndex);
             if(currIndex!=index++){
                return true;
            }
